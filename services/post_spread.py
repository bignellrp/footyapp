...
def update_result(values):
    ...
    cell = ws_results.find(next_wednesday) #Find the cell containing next wednesdays date
    row = cell.row
    ...
    ws_results.update(range, values, major_dimension='ROWS', value_input_option='USER_ENTERED')
...
def append_result(values):
    ...
    ws_results.append_row(values, value_input_option='USER_ENTERED')
    return wipe_tally() #Wipe tally once teams posted to the results page
...
def swap_player(player):
    ...
    player_current = ws_players.find(player[0]) #Find the Current Players name and the row
    player_new = ws_players.find(player[1]) #Find the New Players name and the row
    clm_total = ws_players.find('Total') #Find the Total column
    ...
    player_current_score = ws_players.acell(player_current_range).value #Grab score cell using range
    player_new_score = ws_players.acell(player_new_range).value #Grab score cell using range
    ...
    player_current = ws_results.find(player[0], in_row=row.row) #Find the Current Players name on the row with a dash
    ...
    col_result_num = ws_results.find('Team ' + team + ' Total') #Find the column with the score from Team A or B
    ...
    team_result = ws_results.acell(team_result_range).value #Grab current score for Team A or B
...
def modify_playing_status(player):
    ...
    cell_name = ws_players.find(player) #Find the Players name and the row
    clm_playing = ws_players.find('Playing') #Find the Playing column
    ws_players.update_cell(cell_name.row, clm_playing.col, 'o')
...
def add_new_player(player):
    ...
    ws_players.append_row(new_player) #Should be a list of name and score
...
def copy_formulas(player):
    ...
    ws_players.update_cell(cell_name.row,clm_wins.col,wins_formula)
    ws_players.update_cell(cell_name.row,clm_draws.col,draws_formula)
    ws_players.update_cell(cell_name.row,clm_losses.col,losses_formula)
    ws_players.update_cell(cell_name.row,clm_score.col,score_formula)
    ws_players.update_cell(cell_name.row,clm_playing.col,'o')
    ws_players.update_cell(cell_name.row,clm_played.col,played_formula)
    ws_players.update_cell(cell_name.row,clm_percent.col,percent_formula)
    ws_players.update_cell(cell_name.row,clm_percent3.col,percent3_formula)
